...
const PRIMARY_TARGET_ROLES = [
  ...
  "Data Governance Programme Manager",
          ...
          <div className="bg-card rounded-xl border border-border shadow-lg w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="p-4 sm:p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Field label="Primary target role (select up to 10)"><MultiSelect options={PRIMARY_TARGET_ROLES} value={editing.primary_target_role || ""} onChange={(arr) => setEditing({ ...editing, primary_target_role: arr.join(", ") })} max={10} placeholder="Target roles" /></Field>
                <Field label="Primary target industry (select up to 10)"><MultiSelect options={PRIMARY_TARGET_INDUSTRIES} value={editing.primary_target_industry || ""} onChange={(arr) => setEditing({ ...editing, primary_target_industry: arr.join(", ") })} max={10} placeholder="Target industries" /></Field>
              </div>
